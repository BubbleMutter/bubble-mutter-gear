...
# -j MASQUERADE 网关代理动态 snat (地址映射)
# 仅用于 nat POSTROUTING
# --to-ports $from-$to 指定端口映射区间
iptables -t nat -A POSTROUTING -i eth+ -o wlan0 -j MASQUERADE
iptables -t nat -A POSTROUTING -i eth+ -o wlan0 -j MASQUERADE --to-ports 10000-20000
