...
int cross_bridge(vector<int> & cost) {
    ...
    while (i >= 3) { // 至少要有4个人才能选方案
        // 过桥方案A
        // (max(cost[0], cost[i]) /* 一次 */) + (cost[0] /* 回城 */) + (max(cost[0], cost[i-1]) /* 第二次 */) + (cost[0] /* 回城 */);
        int planA = cost[i] + cost[i-1] + cost[0]*2;
        // 过桥方案B
        // (max(cost[0], cost[1]) /* 一次 */) + (cost[0] /* 回城 */) + (max(cost[i], cost[i-1]) /* 第二次 */) + (cost[1] /* 回城 */);
    ...
    if (i == 2) { // 还剩下3个人
        // max(cost[0], cost[1] /* 一次 */) + cost[0] /* 回城 */ + max(cost[0], cost[2])
